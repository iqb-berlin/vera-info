...
```{dot}
digraph {
    "Lese-Trainer und Lese-Sportler Fangen auf ein vereinbartes zeichen hin an,den text synchron vorzulesen"[
        shape=cluster
    ]
    ...
    "Lese-Trainer und Lese-Sportler Fangen auf ein vereinbartes zeichen hin an,den text synchron vorzulesen" -> "Kein Fehler des Sportlers"
    "Lese-Trainer und Lese-Sportler Fangen auf ein vereinbartes zeichen hin an,den text synchron vorzulesen" ->
    Subgraph cluster {
        label="Bei Fehler des Sportlers"
        color=red
        style=filled
        "Erfolgreiche Selbstverbesserung innerhalb vier Sekunden"[
            shape=cluster
            color=white
            style=filled
        ]
        "Verbesserung durch Trainer nach vier Sekunden Neubeginn am Satzanfang"[
            shape=cluster
            color=white
            style=filled
        ]
    ...
    "Erfolgreiche Selbstverbesserung innerhalb vier Sekunden" -> "Lob des Trainers"
    "Fühlt sich der Sportler beim Lesen sicher, gibt er dem Trainer ein Zeichen. Trainer lobt und liest mit. Sportler liest alleine vor"[
        shape=cluster
    ]
    "Lob des Trainers" -> "Fühlt sich der Sportler beim Lesen sicher, gibt er dem Trainer ein Zeichen. Trainer lobt und liest mit. Sportler liest alleine vor"
    "Fehler des Sportlers"[
        shape=cluster
    ]
    "Fehler des Sportlers" -> Subgraph cluster {
        label="Bei Fehler des Sportlers"
        color=red
        style=filled
        "Erfolgreiche Selbstverbesserung innerhalb vier Sekunden"[
    ...
    "Verbesserung durch Trainer nach vier Sekunden Neubeginn am Satzanfang" -> "Fühlt sich der Sportler beim Lesen sicher, gibt er dem Trainer ein Zeichen. Trainer lobt und liest mit. Sportler liest alleine vor" -> "Fehler des Sportlers"
    "Kein Fehler des Sportlers "[
        shape=cluster
    ]
    "Kein Fehler des Sportlers" -> "Kein Fehler des Sportlers"
    "Fühlt sich der Sportler beim Lesen sicher, gibt er dem Trainer ein Zeichen. Trainer lobt und liest mit. Sportler liest alleine vor" -> "Kein Fehler des Sportlers "
}
```
